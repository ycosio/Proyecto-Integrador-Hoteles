  ...
  if(peticion)
      {
        ...
        peticion.send('funcion=' + funcion + '&campos=*&tabla=reservacion&valores=' + seleccionadoo + '&id=' + idUser);      
      }
}
...
function muestraContenido(){
 ...
 function eliminarSeleccionado(element,user)
 {
  if(confirm("¿Cancelar reservacion?"))
    {
      seleccionadoo=element;
      idUser=user;
      descargarArchivo('eliminaReservacion');
      alert("Reservacion cancelada " + seleccionadoo);
    }
}
 </script>
</head>
...
//------------------ SENTENCIA SQL -------------------\\ 
$sql = "SELECT id_reservacion,fechallegada, hotel,nombre,fechaSalida from allreservaciones where id_cliente = ".$idUser." ORDER BY fechallegada desc;";
//------------------ SENTENCIA SQL -------------------\\ 
// echo $sql."<br>";
...
$res = mysqli_query($mysqli, $sql);
   ...
   $fechaSalida = fechaesp(date_format(new DateTime($newArray['fechaSalida']), 'Y-m-d'));
	 ...
	 echo "  <div class='list-group'>
      ...
      <button class='btn btn-danger' type='submit' name='eliminar' value='Cancelar reservacion' onclick='eliminarSeleccionado($id,$idUser);' ><span class='glyphicon glyphicon-remove'></span> Cancelar reservación</button>
	  </a>
	</div>";
